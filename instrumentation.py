...
def compute_precision_recall_pl(pseudo_labels, y_obs, y_true):
    '''
    pseudo_labels: (N, C), binary matrix where 1 indicates a pseudo label
    y_true: (N, C), binary matrix where 1 indicates the true label
    '''
    # Convert to binary matrices if not already (for safety)
    pseudo_labels = (pseudo_labels > 0).astype(int)
    y_true = (y_true > 0).astype(int)
    
    # True positive pseudo-labels (pseudo-labels that match true labels)
    true_positives = np.sum((pseudo_labels == 1) & (y_true == 1)) - np.sum((pseudo_labels == 1) & (y_obs==1))

    # Total number of pseudo-labels (whether correct or not)
    total_pseudo_labels = np.sum(pseudo_labels) - np.sum((pseudo_labels == 1) & (y_obs==1))

    # Total number of true labels
    total_true_labels = np.sum(y_true) - np.sum(y_obs)

    # Precision: % of correct pseudo-labels over total pseudo-labels
    precision = (true_positives / total_pseudo_labels) * 100 if total_pseudo_labels > 0 else 0

    # Recall: % of correct pseudo-labels over true labels
    recall = (true_positives / total_true_labels) * 100 if total_true_labels > 0 else 0 
